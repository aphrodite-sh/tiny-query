...
> extends BaseChunkIterable<T> {
  ...
  async *[Symbol.asyncIterator](): AsyncIterator<readonly T[]> {
    let chunk: T[] = [];
    for await (const piece of this.source) {
      chunk.push(piece);
...
export class PromiseSourceSingleChunkIterable<T> extends BaseChunkIterable<T> {
  ...
  *[Symbol.iterator](): Iterator<readonly T[]> {
    throw new Error("Promises can not be made synchronous");
  }
}
...
export class MappedChunkIterable<TIn, TOut> extends BaseChunkIterable<TOut> {
  ...
  *[Symbol.iterator](): Iterator<readonly TOut[]> {
    throw new Error(
      "Async map functions functions can not be made synchronous. Use SyncMappedChunkIterable."
    );
  }
}
...
export class GroupByChunkIterable<T, Tv> extends BaseChunkIterable<[Tv, T[]]> {
  ...
  *[Symbol.iterator](): Iterator<readonly [Tv, T[]][]> {
    let all: T[] = [];
    for (const chunk of this.source) {
      all = all.concat(chunk);
    }
